using RoutingExample.Custom_Contraints;

var builder = WebApplication.CreateBuilder(args);

builder.Services.AddRouting(options => options.ConstraintMap.Add("monthsss", typeof(MonthCustomContraints)));
...
app.UseEndpoints(endpoints =>
{
   ...
   /* endpoints.Map("sale-reports/{year:int:min(1900)}/{month:regex(^(apr|jul|jan)$)}", async (context) =>
    ...
    endpoints.Map("sale-reports/{year:int:min(1900)}/{month:monthsss}", async (context) =>
    {
        int year = Convert.ToInt32(context.Request.RouteValues["year"]);
        string month = Convert.ToString(context.Request.RouteValues["month"]);
        await context.Response.WriteAsync($"Sale report date: {year} - {month}");
